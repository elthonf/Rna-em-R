...
execCenario <- function (idCenario){
    ...
    treina.rede[[idCenario]] = emf.rna.read.csv.files(
        ...
        qtIn = dim(treina.X[[idCenario]])[2], qtHid = qtde.neuronios[idCenario] + 2, qtOut = 1, qtRec = 3,
    ...
    valida.rede[[idCenario]] = emf.rna.read.csv.files(
        ...
        qtIn = dim(valida.X[[idCenario]])[2], qtHid = qtde.neuronios[idCenario] + 2, qtOut = 1, qtRec = 3,
    ...
    valida.rede[[idCenario]]$dynamic$R = treina.rede[[idCenario]]$dynamic$R;
    ...
    #LOOP de treinamento
    alpha.min = alpha.max = 0.01;
    for(i in 1:10000){
        if(i == 1000){ #Diminui o Alfa após algumas execucoes
            alpha.min = alpha.max = 0.005;
        }
        if(i == 3000){ #Diminui o Alfa após algumas execucoes
            alpha.min = alpha.max = 0.003;
        }
        if(i == 5000){ #Diminui o Alfa após algumas execucoes
            alpha.min = alpha.max = 0.001;
        }
        if(i == 7000){ #Diminui o Alfa após algumas execucoes
            alpha.min = alpha.max = 0.0005;
        }
        if(i == 9000){ #Diminui o Alfa após algumas execucoes
            alpha.min = alpha.max = 0.0001;
        }
        lastET = treina.rede[[idCenario]]$dynamic$ET;
        treina.rede[[idCenario]]$dynamic = emf.rna.backward.recorrencia(rna = treina.rede[[idCenario]], alpha.min = alpha.min, alpha.max = alpha.max);
...
write.csv( x = log.tab, file = "dados/RecExt.log.tab.csv" )
write.csv( x = log.resumo, file = "dados/RecExt.log.resumo.csv" )

View( log.resumo )
save.image("dados/RecExt Exec.RData")
