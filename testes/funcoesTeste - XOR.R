...
netXOR = emf.rna.read.csv.files( qtIn = 2, qtHid = 2, qtOut = 1, qtRec = 0,
...
for( i in 1:1000 ){
    netXOR1batelada$dynamic = emf.rna.backward.lote( rna = netXOR1batelada, alpha = runif(1, min=0.1, max = 0.9));
...
for( i in 1:1000 ){
    netXOR1padrao$dynamic = emf.rna.backward.padrao( rna = netXOR1padrao, padrao = 1:qtde, alpha.min= 0.1, alpha.max = 0.9);
